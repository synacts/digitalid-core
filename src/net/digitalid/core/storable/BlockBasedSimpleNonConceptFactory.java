...
/**
 * This class implements the methods that all simple non-concept classes whose storable mechanisms use a {@link Block block} share.
 * 
 * @author Kaspar Etter (kaspar.etter@digitalid.net)
 * @version 1.0
 */
@Immutable
public abstract class BlockBasedSimpleNonConceptFactory<O extends Storable<O>> extends SimpleNonConceptFactory<O> {
     ...
     */
    protected BlockBasedSimpleNonConceptFactory(@Nonnull @Loaded SemanticType type) {
        super(type, COLUMN);
    }
    ...
    @Override
    @NonCommitting
    public final @Nullable O getNullable(@Nonnull ResultSet resultSet, int columnIndex) throws SQLException {
        try {
