...
public final class SecretCommitment extends Commitment {
     ...
     */
    SecretCommitment(@Nonnull HostIdentity host, @Nonnull Time time, @Nonnull Element value, @Nonnull PublicKey publicKey, @Nonnull Exponent secret) throws RequestException {
        super(host, time, value, publicKey);
        
        if (!publicKey.getAu().pow(secret).equals(value)) { throw new RequestException(RequestErrorCode.INTERNAL, "The secret does not match the commitment."); }
        this.secret = secret;
    }
